		...
		it("/subscriptions/:id (GET) should return a subscription by id", () => {
			...
			return request(app.getHttpServer())
				...
				.then((res) => {
					if (res.status !== 200) {
						throw new Error(
							`GET /subscriptions/:id returned ${res.status}: ${JSON.stringify(res.body)}`,
						);
					}
					expect(res.body).toEqual(mockSubscriptionResponse);
				});
		});
		...
		it("/subscriptions/:id (PATCH) should update a subscription", () => {
			...
			return request(app.getHttpServer())
				...
				.then((res) => {
					if (res.status !== 200) {
						throw new Error(
							`PATCH /subscriptions/:id returned ${res.status}: ${JSON.stringify(res.body)}`,
						);
					}
					expect(res.body).toEqual({ ...waitlistedSubscription, status: "cancelled" as const });
				});
		});
		...
		it("/subscriptions/:id (DELETE) should cancel a subscription", () => {
			...
			return request(app.getHttpServer())
				...
				.then((res) => {
					if (res.status !== 200) {
						throw new Error(
							`DELETE /subscriptions/:id returned ${res.status}: ${JSON.stringify(res.body || res.text)}`,
						);
					}
